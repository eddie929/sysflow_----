  ...
  methods: {
        ...
        .then(() => {
          del_linesqlwherebyid({ id: rows[index].id }).then((res) => {
            ...
            if (rows[index].column_name === "") {
              ...
              for (let i in this.tableDisable) {
                if (
                  this.tableDisable[i].name === rows[index].column_name.name
                ) {
              ...
              if (!this.tableDisable || this.tableDisable.length < 1) {
                ...
                // 按需屏蔽选项
                for (const option of this.tablecolumn) {
                  const found = this.tableDisable.find(
                    (item) => item === option
                  );
                  if (found) {
    ...
    selectchange(val) {
      ...
      if (this.trigger === true) {
        ...
        const vm = this;
        for (let i in this.tablecolumn) {
          if (this.tablecolumn[i].name === this.previous[0].name) {
            vm.tablecolumn[i] = {
              name: vm.tablecolumn[i].name,
              type: vm.tablecolumn[i].type,
              disabled: false,
            };
          }
        }
        for (let i in this.tableDisable) {
          if (this.tableDisable[i].name === this.previous[0].name) {
